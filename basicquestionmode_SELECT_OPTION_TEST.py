...
class MainWindow(QMainWindow):
    ...
    def animate(self):
        try:
            ...
            self.animation = animation.FuncAnimation(self.figure,self.animate_loop,frames=np.arange(0, CalcTimeTaken(float(self.veloinp.text()), float(self.angleinp.text()), float(self.elevinp.text())), 0.01),interval=10,repeat=False) 
            self.canvas.draw()
        except:
            print("Error")
